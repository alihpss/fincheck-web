...
export function ColorsDropdowninput({
  ...
  const [selectedColor, setSelectedColor] = useState<null | Color>(() => {
    ...
    return colors.find((color) => color.bg === value) ?? null;
  });
    ...
    <div>
      <DropdownMenu.Root>
        <DropdownMenu.Trigger>
          <button
            className={cn(
              "bg-white w-full rounded-lg border border-gray-500 px-3 h-[52px] text-gray-800 placeholder-shown:pt-0 focus:border-gray-700 transition-all outline-none text-left relative",
